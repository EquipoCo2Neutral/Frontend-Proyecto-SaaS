...
import InviteTenantModal from "@/views/Inquilinos/InviteTenantModal";
...
export default function TenantInfo({ inquilinos }: Props) {
  ...
  const handleChangeEstado = async (estado: boolean) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: ["editTenant", inquilinoActual.inquilinoId],
      });
    } catch (error) {
  ...
  const handleCreateSuscripcion = async (data: {
    estado: boolean;
    diasActivo: number;
    planId: string;
  }) => {
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <p className="text-gray-700">
                ...
                {inquilino.correoInquilino}
              </p>
            </div>
            <div className="space-y-2">
              <p className="text-gray-700">
                <span className="font-semibold">Dirección:</span>{" "}
                {inquilino.direccionInquilino}
              </p>
              <p className="text-gray-700">
          ...
          <div className="">
            <div className="flex justify-between items-center">
              ...
              <button
                className="bg-gray-900 text-gray-100 text-xl w-10 h-10 rounded-full hover:bg-gray-700 transition duration-200"
                onClick={() =>
                  navigate(location.pathname + "?inviteTenant=true")
                }
              >
                +
              </button>
            ...
            <div className="flex space-x-4 overflow-x-auto p-4">
              {personas.map((persona, index) => (
                <div
                  key={index}
                  className="relative w-64 bg-white shadow-lg rounded-2xl p-4 border border-gray-200 flex-shrink-0"
                >
                  ...
                  <div className="mt-8 space-y-2">
                    ...
                    <p className="text-gray-400 text-sm">
                      Correo contacto: {persona.usuario.correoUsuario}
                    </p>
                  </div>
                </div>
              ))}
